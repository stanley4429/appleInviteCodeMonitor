...
# 获取邀请码
async def getAppDownCode(session, channel):

    try:
        async with session.post("https://api.mrb6ns.cn/front/getAppDownCode?%s" % time.time(),
                                data={'code': channel['c_randStr'], 'ip': ''}, ssl=False) as resp:
            try:
                html = await resp.json()
                if int(html['code']) == 1:
                    return html
                elif int(html['code']) == 0:
                    notify_ending('%s: \n%s\n#请求异常  (code == %s): \n%s'
                                  % (channel['c_name'], channel['c_link'], html['code'], html['msg']), isPin=True)
                else:
                    notify_ending('%s: \n%s\n#请求异常  (code == %s): \n%s'
...
async def getInvitationCode(session, channel):
    try:
        async with session.post("https://api.techchitu.com/front/getInvitationcode?%s" % time.time(),
                                data={'randStr': channel['c_randStr']}, ssl=False) as resp:
            try:
                html = await resp.json()
                if int(html['code']) == 1:
...
async def appleInvite(session, params, channel):
    url = params["data"]
    if u'itms-beta://beta.itunes.apple.com/' in url:
       url = url.replace(u'itms-beta://beta.itunes.apple.com/', u'https://testflight.apple.com/')
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            for c in channels:
                if int(c['c_valid']) == 1:
                    if 'techchitu' in c['c_link']:
                        params = await getInvitationCode(session, c)
                    else:
                        params = await getAppDownCode(session, c)
